...
(function ($) {
    ...
    function getUrlParamValues(params) {
        ...
        // Special case for the pluginfile.php URLs in h5pactivity edit mode to find contextid.
        if (
            params == 'contextid' &&
            urlParams.has('url') &&
            urlParams.get('url').includes('pluginfile.php')
        ) {
            let relativepath = urlParams.get('url').split('pluginfile.php/')[1];
            let contextid = relativepath.split('/')[0];
            return contextid;
        }
